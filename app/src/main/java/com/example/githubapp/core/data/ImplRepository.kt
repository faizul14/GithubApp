...
class ImplRepository @Inject constructor(private val remoteDataSource: RemoteDataSource) : IRepository {
    override fun getDataUser(): LiveData<List<ModelDataUser>> {
        return Transformations.map(remoteDataSource.getDataUser()){
            DataMapper.mapResponseDataUserToModelDataUser(it.responseDataUser as List<ResponseDataUserItem>)
        }
    }
    ...
    override fun getRepoUser(username: String): LiveData<List<ModelRepoUser>> {
        return Transformations.map(remoteDataSource.getRespoUser(username)){
            DataMapper.mapResponseRepoUsetToModelRepoUser(it.responseRepoUser as List<ResponseRepoUserItem>)
        }
    }
}